...
void main() {
  ...
  group('RemoteTimetableRepository', () {
    test('lists timetables with the stored bearer token', () async {
      ...
      final client = ApiClient(
        ...
        client: MockClient((request) async {
          ...
          expect(request.url.toString(), 'http://example.com/users/${user.id}/timetables');
          expect(
    ...
    test('saves, renames, and deletes timetables through the backend API', () async {
      SharedPreferences.setMockInitialValues({'auth.token': 'server-token'});
      final user = _sampleUser();
      final calls = <String>[];
      final client = ApiClient(
    ...
    test('surfaces a readable message when the backend is unreachable', () async {
      SharedPreferences.setMockInitialValues({'auth.token': 'server-token'});
